...
> ## ✅ 执行状态：已全部完成（2026-08-14，inline 逐任务执行）
>
> - **Task 0–8 全部完成**：8 个验收标准全部满足并验证（当时 25/25 测试，demo 运行干净）。
> - **执行期修正**（写代码时发现，均已落地）：
>   - `delphi_project` MCP 编译器版本不可控（错配 XE7/dcc32 与 Delphi 13 库）→ 改为直接调用 Delphi 13 `dcc32`；DUnitX 源码路径 `-U` 指向 `Studio\37.0\source\DunitX`。
>   - Delphi 13 无 `DUnitX.Loggers.Xml`（拆分为 JUnit/NUnit/xUnit）与 `TDUnitX.ConsoleLogger`（实际为 `TDUnitXConsoleLogger`）→ runner 相应调整。
>   - 测试中 `ETestFailure` 不存在 → 用 `Exception`。
>   - 事件退订从"比较 procedure-reference 字段"（Delphi 解析歧义 E2014/E2035）改为**注册 token**（整数）删除。
>   - 类插件在 Apply 后需保持存活（清理闭包捕获 Self）→ `MountUnderFreshFiber` 首个注册 `Effects.AddDisposable(LPlugin)`。
>   - `RescanPending` 快照后**锁外**激活（`DoActivate` 会重入触发 `OnServiceAdded`）。
> - **MVP 之后的项目演进**（本计划之外，详见设计 spec §13–§17）：二期 `Oasis.Otl` 异步事件、三期 `Oasis.Bpl` 动态加载、四期 `Context.Reload`、单插件 `Reload/Unload`、依赖失效级联、JSON 配置（env 分层/类型化）、`Oasis.UI` marshaling 桥、失败可见性修复、5 个运行时包 + `build.cmd`、7 个 demo。当前状态：**42 测试全绿，v0.2.0 已发布**（https://github.com/yslmyr/oasis_framework）。
...
- [x] **Step 2: Confirm git baseline exists** (the pre-existing impl is committed at `8051cc8`). No action unless missing.
...
- [x] **1. Context tree** — `Oasis.Context.Tests`: `Dispose_Tears_Down_Fibers_In_Reverse_Order`, `Fork...`. PASS when tests green.
- [x] **2. Effects** — `Oasis.Effects.Tests`: LIFO, fault aggregation, idempotent, manual dispose. PASS when 4 green.
- [x] **3. Services** — `Oasis.Services.Tests`: register/get/resolve, parent chain, shadowing, not-found. PASS when 6 green.
- [x] **4. Dependency activation** — `Oasis.Hosting.Tests`: `Dependency_Registered_Later_Activates_Consumer`, `Missing_Dependency_Leaves_Plugin_Pending`. PASS when green.
- [x] **5. Events** — `Oasis.Events.Tests`: auto-unsub on scope dispose, emit order, isolation, waterfall short-circuit, bubble. PASS when 6 green.
- [x] **6. Plugin authoring** — class (`TRecordingPlugin`) + closure + returned-disposer forms all exercised in context tests. PASS when green.
- [x] **7. Host** — `Start`/`Shutdown`, lifecycle events (`Oasis.Hosting.Tests`). PASS when green.
- [x] **8. Console demo runs** — `ConsoleDemo.exe` outputs expected, no leaks. PASS.

- [x] **Final commit (if any fixups)**
